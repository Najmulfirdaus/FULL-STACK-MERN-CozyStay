...
function ListingPage1() {
  let [error,seterror]=useState({})
   ...
   function validation(){
    let temporary={}
    if(!tittle || title.trim()===''){
      temporary.title='Title can be null'
    }
    if(!description||description.length<=10){
      temporary.description='description should be more than 8 charachter'
    }
    if(!image){
      temporary.image='select image'
    }
    if(!city && !isNaN(city)){
      temporary.city='city shoul character'
    }
    if(!rent && Number(rent)>=0 && isNaN(rent)){
      temporary.rent='rent should be nuber'
    }
   }
